...
- **Everything it stores gets tagged automatically.** A domain (`homelab`/`personal`/`obsidian`/`test`) and up to 3 topic tags from a fixed list — no extra step for you, it rides along with the same thinking pass that figures out what you meant. Keeps things organized as the memory grows instead of turning into one big undifferentiated pile. See `architecture/tagging.md` for the full list.
